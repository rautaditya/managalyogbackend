...
function drawTable(doc, data, tableTop, L, R) {
  ...
  const advanceAmt = Number(data.advance_amount || 0);
  const grandTotal = subtotal + taxAmount;
  const balanceDue = grandTotal - advanceAmt;
  ...
  // In Word based on Balance Due
  const inWordText  = numberToWords(Math.floor(balanceDue)) + ' Rupees Only';
  const inWordBodyW = TR - TL - 82;
  const inWordBodyH = doc.font('Helvetica').fontSize(9)
    .heightOfString(inWordText, { width: inWordBodyW });
  const inWordH = Math.max(28, inWordBodyH + 16);
  ...
  const rowY = [];
  let cur = tableTop + headerH;
  itemHeights.forEach(h => { rowY.push(cur); cur += h; });

  const sumStartY  = cur;
  const sumRow0Y   = cur;               // Subtotal
  const sumRow1Y   = cur + sumRowH;     // GST
  const sumRow2Y   = cur + sumRowH * 2; // Grand Total
  const sumRow3Y   = cur + sumRowH * 3; // Advance Paid (-)
  const sumRow4Y   = cur + sumRowH * 4; // Balance Due
  const inWordY    = cur + sumRowH * 5;
  const tableBottom = inWordY + inWordH;
  ...
  // Grand Total row — light orange
  doc.rect(TL, sumRow2Y, TR - TL, sumRowH).fill('#FFF4F0');
  // Balance Due row — stronger orange highlight
  doc.rect(TL, sumRow4Y, TR - TL, sumRowH).fill('#FFEDE5');
  ...
  const hLines = [tableTop, tableTop + headerH];
  rowY.forEach((y, i) => hLines.push(y + itemHeights[i]));
  [sumRow0Y, sumRow1Y, sumRow2Y, sumRow3Y, sumRow4Y, inWordY, tableBottom]
    .forEach(y => hLines.push(y));

  hLines.forEach(y => doc.moveTo(TL, y).lineTo(TR, y).stroke());

  // Item section — all column verticals
  [cSr, cDesc, cQty, cRate, cAmt, TR].forEach(x => {
    doc.moveTo(x, tableTop).lineTo(x, sumStartY).stroke();
  });

  // Summary section — left border + rate/amount dividers only
  [cSr, cRate, cAmt, TR].forEach(x => {
    doc.moveTo(x, sumStartY).lineTo(x, inWordY).stroke();
  });

  // In Word row — outer borders only
  [cSr, TR].forEach(x => {
    doc.moveTo(x, inWordY).lineTo(x, tableBottom).stroke();
  });
  ...
  doc.text('#', cSr + 2, hMid, {
    width: cDesc - cSr - 4, align: 'center', lineBreak: false
  });
  doc.text('ITEM DESCRIPTION', cDesc + 5, hMid, {
    width: descWidth, align: 'center', lineBreak: false
  });
  doc.text('QTY', cQty + 3, hMid, {
    width: cRate - cQty - 6, align: 'center', lineBreak: false
  });
  doc.text('Rate/Qty', cRate + 4, hMid, {
    width: cAmt - cRate - 8, align: 'center', lineBreak: false
  });
  doc.text('Total', cAmt + 3, tableTop + 6, {
    width: TR - cAmt - 6, align: 'center', lineBreak: false
  });
  doc.text('Amount', cAmt + 3, tableTop + 17, {
    width: TR - cAmt - 6, align: 'center', lineBreak: false
  });
  ...
  items.forEach((item, i) => {
    ...
    doc.font('Helvetica-Bold').fontSize(9).fillColor(BLACK)
      .text(boldLine, cDesc + 6, ry + 10, { width: descWidth, lineBreak: true });

    if (restText) {
      const bH = doc.font('Helvetica-Bold').fontSize(9)
        .heightOfString(boldLine, { width: descWidth });
      doc.font('Helvetica').fontSize(9).fillColor(BLACK)
        .text(restText, cDesc + 6, ry + 10 + bH, { width: descWidth, lineBreak: true });
    }
    ...
    doc.font('Helvetica').fontSize(9).fillColor(BLACK)
      ...
      .text(fmtNum(item.rate), cRate + 4, midY, {
        width: cAmt - cRate - 8, align: 'right', lineBreak: false
      })
      .text(fmtNum(amount), cAmt + 4, midY, {
        width: TR - cAmt - 8, align: 'right', lineBreak: false
      });
  });

  // ── PASS 5 — SUMMARY ROWS ─────────────────────────────────────────────────
  const sumDefs = [
    { y: sumRow0Y, label: 'Sub Total',                        val: subtotal,   bold: false, color: BLACK },
    { y: sumRow1Y, label: `GST ${data.tax_rate || 0}%`,       val: taxAmount,  bold: false, color: BLACK },
    { y: sumRow2Y, label: 'Grand Total',                      val: grandTotal, bold: true,  color: BLACK },
    { y: sumRow3Y, label: 'Advance Paid (-)',                  val: advanceAmt, bold: false, color: '#DC2626' },
    { y: sumRow4Y, label: 'Balance Due',                      val: balanceDue, bold: true,  color: ORANGE  },
  ];

  sumDefs.forEach(({ y, label, val, bold, color }) => {
    const mY = y + Math.floor((sumRowH - 9) / 2);
    ...
    doc.font(bold ? 'Helvetica-Bold' : 'Helvetica')
      ...
      .fillColor(BLACK)
      .text(label, cRate + 4, mY, {
        width: cAmt - cRate - 8, align: 'left', lineBreak: false
      });
  ...
  doc.font('Helvetica').fontSize(9).fillColor(BLACK)
    .text(inWordText, TL + 74, inWordY + 8, {
      width: inWordBodyW, lineBreak: true
    });
