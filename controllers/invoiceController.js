...
const getFullInvoice = async (id) => {
  const [invRows] = await pool.query(
    `SELECT 
        i.*,
        COALESCE(s.name, i.client_name) AS display_name,
  ...
  return {
    ...
    site: {
      id: inv.site_id,
      name: inv.display_name,
      address: inv.site_address || inv.address,
      owner_name: inv.site_owner_name,
      phone: inv.site_phone || inv.phone,
      gst_number: inv.site_gst_number || inv.gst_number,
      project_name: inv.site_project_name,
    },
    items,
  };
};
...
const createInvoice = async (req, res) => {
  ...
  try {
    ...
    const [result] = await conn.query(
      `INSERT INTO invoices
        (invoice_number, site_id, client_name, address, phone, gst_number,
         subtotal, tax_rate, tax_amount, total, advance_amount, status,
         due_date, notes, date, created_by)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        invNumber,
        site_id || null,
    ...
    console.log("BACKEND INVOICE BODY:", req.body);
  ...
  try {
    ...
    if (items) {
      ...
      await conn.query(
        `UPDATE invoices
         SET subtotal=?, tax_rate=?, tax_amount=?, total=?, status=?,
             due_date=?, notes=?, date=?, client_name=?, address=?, phone=?,
             gst_number=?, advance_amount=?
         WHERE id=?`,
        [
          itemsTotal,
          tr,
